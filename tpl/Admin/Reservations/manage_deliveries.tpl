...
<div id="page-manage-reservations" class="admin-page">
	<div>
		<h1>{translate key=ManageDeliveries}</h1>
	</div>

	<div class="panel panel-default filterTable" id="filter-reservations-panel">
		<div class="panel-heading"><span class="glyphicon glyphicon-filter"></span> {translate key="NIUFilter"} {showhide_icon}</div>
			...
			<form id="filterForm" role="form">
				<div class="form-group filter-user {$groupClass}">
					<input id="userId" type="text" class="form-control" value="{$UserIdFilter}"
						   placeholder="{translate key=UserAuthorizedNIU}"/>
	...
	<table class="table admin-panel" id="reservationTable">
		...
		<tbody>
		  {foreach from=$reservations item=reservation}
			  		{include file='Admin/Reservations/technician_delivery.tpl' reservation=$reservation}
		  {/foreach}
		</tbody>
		<tfoot>
		<tr>
    ...
    <script type="text/javascript">
		...
		$(document).ready(function () {
			...
			var resOpts = {
				...
				reservationUrlTemplate: "{$Path}delivery.php?{QueryStringKeys::REFERENCE_NUMBER}=[refnum]",
